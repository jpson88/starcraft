...
public class StrategyManager {
	...
	void setVariables(){
		...
		if (myRace == Race.Protoss) {
			...
			// 공격 모드로 전환하기 위해 필요한 최소한의 유닛 숫자 설정
                // 공격을 시작하기위해 필요한 최소한의 시즈탱크 유닛 숫자 
			if(enemyRace == Race.Terran) {
				myCombatUnitType1 = UnitType.Terran_Marine;
				myCombatUnitType2 = UnitType.Terran_Goliath;
				myCombatUnitType3 = UnitType.Terran_Battlecruiser;
				necessaryNumberOfCombatUnitType1 = 0;                      // 공격을 시작하기위해 필요한 최소한의 마린 유닛 숫자 
				necessaryNumberOfCombatUnitType2 = 0;                       // 공격을 시작하기위해 필요한 최소한의 메딕 유닛 숫자 
				necessaryNumberOfCombatUnitType3 = 12;                       // 공격을 시작하기위해 필요한 최소한의 시즈탱크 유닛 숫자 
				buildOrderArrayOfMyCombatUnitType = new int[]{3,3,3,3,2,2,2,2,2};
				necessaryUpgradeType1 = UpgradeType.Terran_Ship_Plating;
				necessaryUpgradeType2 = UpgradeType.Terran_Ship_Weapons;
				necessaryUpgradeType3 = UpgradeType.Charon_Boosters;
				...
				necessaryNumberOfCombatUnitType1 = 0;                      // 공격을 시작하기위해 필요한 최소한의 마린 유닛 숫자 
				necessaryNumberOfCombatUnitType2 = 12;                       // 공격을 시작하기위해 필요한 최소한의 메딕 유닛 숫자 
				necessaryNumberOfCombatUnitType3 = 4;       
				buildOrderArrayOfMyCombatUnitType = new int[]{3,2,2,2,2};
				necessaryUpgradeType1 = UpgradeType.Terran_Vehicle_Plating;
				necessaryUpgradeType2 = UpgradeType.Terran_Vehicle_Weapons;
				necessaryUpgradeType3 = UpgradeType.Charon_Boosters;
			}
			// 공격 유닛 생산 순서 설정
			buildOrderArrayOfMyCombatUnitType = new int[]{3,2,2,2,2}; 	// 마린 마린 마린 메딕 시즈탱크 ...
			nextTargetIndexOfBuildOrderArray = 0; 			    // 다음 생산 순서 index
			
			// 특수 유닛 종류 설정 
			mySpecialUnitType1 = UnitType.Terran_Science_Vessel;	
			if(enemyRace == Race.Protoss) {
				mySpecialUnitType2 = UnitType.Terran_Ghost;
			}
			else {
			...
			// 공격 모드로 전환하기 위해 필요한 최소한의 유닛 숫자 설정
			necessaryNumberOfSpecialUnitType1 = 2;		             	 
			necessaryNumberOfSpecialUnitType2 = 12;	
			if(enemyRace == Race.Protoss) {
				necessaryNumberOfSpecialUnitType2 = 6;
			}
			
			// 특수 유닛을 최대 몇개까지 생산 / 전투참가 시킬것인가
			maxNumberOfSpecialUnitType1 = 2;  
			maxNumberOfSpecialUnitType2 = 24;  
			...
			necessaryTechType2 = TechType.EMP_Shockwave;
			//necessaryTechType3 = TechType.Yamato_Gun;
			if(MyBotModule.Broodwar.enemy().getRace() == Race.Protoss) {
				necessaryTechType3 = TechType.Lockdown;
			}
			if(myPlayer.getStartLocation().getX() > 64 && myPlayer.getStartLocation().getY() > 64) {
				quarter = "SE";
			}
			else if(myPlayer.getStartLocation().getX() > 64 && myPlayer.getStartLocation().getY() < 64) {
				quarter = "NE";
			}
			else if(myPlayer.getStartLocation().getX() < 64 && myPlayer.getStartLocation().getY() > 64) {
				quarter = "SW";
	...
	public void setInitialBuildOrder() {
		...
		if (MyBotModule.Broodwar.enemy().getRace() == Race.Protoss) {
			...
			//BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Siege_Tank_Tank_Mode); // 25
			...
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_SCV);
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Supply_Depot);
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UpgradeType.Terran_Ship_Plating);
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_SCV,
					BuildOrderItem.SeedPositionStrategy.FirstExpansionLocation);
			...
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_SCV,
					BuildOrderItem.SeedPositionStrategy.FirstExpansionLocation);
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UpgradeType.Terran_Ship_Weapons);
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Supply_Depot); 
...
/*
	...
	public void update() {
		MyBotModule.Broodwar.sendText("DIRECTION : " + quarter);
		/// 변수 값을 업데이트 합니다
		updateVariables();
	...
	public void executeCombat() {
		...
		// 공격을 시작한 후에는 공격을 계속 실행하다가, 거의 적군 기지를 파괴하면 Eliminate 시키기를 합니다 
			...
			if (isTimeToStartDefense() ) {
				combatState = CombatState.defenseMode;
				//MyBotModule.Broodwar.sendText("ATTACK -> DEFENSE");
			}	
						/// 적군을 Eliminate 시키는 모드로 전환할지 여부를 판단합니다 
			else if (isTimeToStartElimination() ) {
				combatState = CombatState.eliminateEnemy;
				//MyBotModule.Broodwar.sendText("ELIMINATE");
			}
			else {
				//MyBotModule.Broodwar.sendText("JUST ATTACK");
				commandMyCombatUnitToAttack();
			}
	...
	boolean isTimeToStartAttack(){
		...
		{	
			// 공격 유닛이 40 이상 있으면
			if ((myCombatUnitType1List.size() + myCombatUnitType2List.size() + myCombatUnitType3List.size() > 23) || (mySpecialUnitType2List.size() > 11) ) {
				/*
	...
	boolean isTimeToStartDefense() {
		...
		if(enemyRace==Race.Terran) {
			if(myCombatUnitType3List.size() <= 2) {
				return true;
			}	
	...
	boolean isTimeToStartElimination(){
		...
		if (enemyKilledCombatUnitCount >= 20 && enemyKilledWorkerUnitCount >= 10 && myPlayer.supplyUsed() > 100 * 2) {
			//MyBotModule.Broodwar.sendText("ELIMINATION CHECKING");
			...
			//MyBotModule.Broodwar.sendText("UNIT AROUND ENEMY BASE: " + myUnitCountAroundEnemyMainBaseLocation);
	...
	void commandMyCombatUnitToDefense(){
		...
		for (Unit selfUnit : MyBotModule.Broodwar.self().getUnits()) {
			if (selfUnit.getType().isWorker()) {
				if(selfUnit.isUnderAttack()) {
					for(Unit someUnit : MyBotModule.Broodwar.getUnitsInRadius(selfUnit.getPosition(), 6 * Config.TILE_SIZE)) {
						if(someUnit.getPlayer() == enemyPlayer) {
							for(Unit unit : myAllCombatUnitList) {
								if(unit == null || unit.exists() == false) {
									selfUnit.attack(someUnit);
								}
								else {
									unit.attack(someUnit);
								}
							}
						}
					}
		...
		for (Unit unit : myAllCombatUnitList) {
			...
			if (unit.getType() == UnitType.Terran_Marine) {
				for(Unit bunker : myDefenseBuildingType1List) {
					...
					int dist = bunker.getDistance(myFirstExpansionLocation.getPosition());
					MyBotModule.Broodwar.sendText("DISTANCE : " + dist);
				}
			}
			...
			if (hasCommanded == false) {
				...
				if (unit.isIdle()) {				
					if (unit.canAttack()) {
						...
						//MyBotModule.Broodwar.sendText("MOVING TO DEFENSE");
						//hasCommanded = true;
					}
					else {
			...
			for (Unit unit : myAllCombatUnitList) {
				...
				if(unit.getType() == UnitType.Terran_Goliath) {
					...
					if(!unit.isAttacking()) {
						...
						for(Unit tank : myCombatUnitType3List) {
							if(tank.getDistance(targetPosition) < min_dist) {
								closest = tank;
								min_dist = tank.getDistance(targetPosition);
							}
						}
						commandUtil.attackMove(unit,closest.getPosition());
						//hasCommanded = true;
					}
					else {
				...
				if (hasCommanded == false) {
					...
					if (unit.isIdle()) {
						...
						if (unit.canAttack() ) {
							commandUtil.attackMove(unit, targetPosition);							
							hasCommanded = true;
							//MyBotModule.Broodwar.sendText("MOVING TO ATTACK / Target Position : " + targetPosition);
						
							if(unit.isStuck()) {
								for(Unit building : myPlayer.getUnits()) {
									if(building.getType() == UnitType.Terran_Factory && building.canLift()) {
										building.lift();
									}			 	
								...
								for(Unit building : myPlayer.getUnits()) {
									if(building.getType() == UnitType.Terran_Factory && building.isLifted()) {
										building.land(building.getTilePosition());
									}			 	
							...
							// 따로 명령을 내린 적이 없으면 다른 공격유닛들과 동일하게 이동하도록 되어있습니다.
								...
								for(Unit myUnit : myCombatUnitType2List) {
									if(myUnit.getHitPoints() == myUnit.getInitialHitPoints() && (myUnit.isMoving() || myUnit.isAttacking())) {
										commandUtil.rightClick(unit, myUnit);
										hasCommanded = true;
										MyBotModule.Broodwar.sendText("FOLLOWING");
									}
								}
							}
						}
	...
	boolean controlSpecialUnitType1(Unit unit) {
		...
		if (unit.getType() == UnitType.Protoss_Observer) {
			...
			// targetPosition 을 적절히 정해서 이동시켜보세요
			for(Unit target_unit : myCombatUnitType2List) {
				if(target_unit.getType() == UnitType.Terran_Goliath && target_unit.isTargetable()) {
					unit.move(target_unit.getPosition());
				}
			}
			...
			if (unit.getEnergy() >= TechType.EMP_Shockwave.energyCost() && myPlayer.hasResearched(TechType.EMP_Shockwave)) {
				
				//Unit targetEnemyUnit = null;
				
				// targetEnemyUnit 을 적절히 정해보세요
				for(Unit targetEnemyUnit : MyBotModule.Broodwar.enemy().getUnits()) {
					if (targetEnemyUnit.getType() == UnitType.Protoss_Dragoon || targetEnemyUnit.isVisible()) {
						unit.useTech(TechType.EMP_Shockwave, targetEnemyUnit);
						hasCommanded = true;
					}
				}
			}	
	...
	boolean controlSpecialUnitType2(Unit unit) {
		...
		else if (unit.getType() == UnitType.Terran_Ghost) {
			
			if (unit.getEnergy() >= TechType.Lockdown.energyCost() && myPlayer.hasResearched(TechType.Lockdown)) {
				
				Unit targetEnemyUnit = null;
				
				// targetEnemyUnit 을 적절히 정해보세요
				for(Unit dragoon : MyBotModule.Broodwar.enemy().getUnits()) {
					if (dragoon.getType() == UnitType.Protoss_Dragoon && dragoon.isVisible() && dragoon.isTargetable() ) {
						unit.useTech(TechType.Lockdown, dragoon);
						hasCommanded = true;
					}
				}
			}	
	...
	public void executeSupplyManagement() {
		...
		if (MyBotModule.Broodwar.self().supplyTotal() < 400) {
			...
			int supplyMargin = 12;
			if(MyBotModule.Broodwar.self().supplyUsed() > 70) {
				supplyMargin = 20;
			}
	...
	void executeUpgradeAndTechResearch() {
		...
		if (myRace == Race.Protoss) {
			...
			if (myPlayer.completedUnitCount(UnitType.Terran_Siege_Tank_Tank_Mode) + myPlayer.completedUnitCount(UnitType.Terran_Siege_Tank_Siege_Mode) >= 1) {
				isTimeToStartUpgradeType1 = true;
			}
			// 업그레이드 / 리서치를 너무 성급하게 하다가 위험에 빠질 수 있으므로, 최소 탱크 2기 생산 후 업그레이드한다
			if (myPlayer.completedUnitCount(UnitType.Terran_Engineering_Bay) > 0
...
/*
			...
			if (myPlayer.completedUnitCount(UnitType.Terran_Covert_Ops) > 0) {
		...
		if (isTimeToStartUpgradeType1) 
		{
			...
			if (myPlayer.getUpgradeLevel(necessaryUpgradeType1) == 1 && myPlayer.getUpgradeLevel(necessaryUpgradeType2) == 1
					&& myPlayer.isUpgrading(necessaryUpgradeType1) == false) {
				BuildManager.Instance().buildQueue.queueAsLowestPriority(necessaryUpgradeType1, false);
			}*/
		}
	...
	void executeTechTreeUpConstruction() {
		...
		if (myRace == Race.Protoss) {
			...
			if (myPlayer.completedUnitCount(UnitType.Terran_Factory) > 4 ) {
				
				// myPlayer.allUnitCount() 으로 제대로 카운트 안되는 경우가 있어서, 별도 카운트
				//int addonBuildingCount = 1;
				//int factoryCount = 1;
				/*
				for(Unit unit : myPlayer.getUnits()) {
					if (unit.getType() == UnitType.Terran_Machine_Shop) {
						addonBuildingCount ++;
						System.out.println(addonBuildingCount);
						MyBotModule.Broodwar.sendText("Machine Shop " + addonBuildingCount + " has the following players:");
						//break;
					}
					...
					if (unit.getType() == UnitType.Terran_Factory) {
						if (unit.isCompleted() && unit.isConstructing()) {
							//addonBuildingCount ++;
							factoryCount++;
							MyBotModule.Broodwar.sendText("Factory " + factoryCount + " has the following players:");
							System.out.println(factoryCount);
						}
					}
				}*/
				//MyBotModule.Broodwar.sendText("Machine Shop ============================================" + myPlayer.completedUnitCount(UnitType.Terran_Machine_Shop) + " has the following players:");
				//MyBotModule.Broodwar.sendText("Factory =================================================" + myPlayer.completedUnitCount(UnitType.Terran_Factory) + " has the following players:");
				/*
				if(myPlayer.completedUnitCount(UnitType.Terran_Factory) >  myPlayer.completedUnitCount(UnitType.Terran_Machine_Shop)) {
					if(MyBotModule.Broodwar.self().minerals() >= 700) {
			...
			if (myPlayer.completedUnitCount(UnitType.Terran_Science_Facility) > 0 ) {
				...
				int addonBuildingCount = 0;
				for(Unit unit : myPlayer.getUnits()) {
					if (unit.getType() == UnitType.Terran_Covert_Ops) {
						addonBuildingCount ++;
						break;
					}
				...
				if (addonBuildingCount == 0
					&& BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Covert_Ops) == 0
					&& ConstructionManager.Instance().getConstructionQueueItemCount(UnitType.Terran_Covert_Ops, null) == 0) 
				{
					BuildManager.Instance().buildQueue.queueAsHighestPriority(UnitType.Terran_Covert_Ops, true);
				}
			}
	...
	public void executeCombatUnitTraining() {
		...
		if (myPlayer.supplyUsed() <= 390 ) 
		{
				...
				if (unit.getType() == producerType) {
					if (unit.isTraining() == false && unit.isMorphing() == false) {
						...
						if (BuildManager.Instance().buildQueue.getItemCount(nextUnitTypeToTrain) == 0) {	
							...
							if (nextUnitTypeToTrain == UnitType.Terran_Siege_Tank_Tank_Mode) {
							...
							else if(nextUnitTypeToTrain == UnitType.Terran_Goliath) {
								if (myPlayer.completedUnitCount(UnitType.Factories) > 0 && myPlayer.completedUnitCount(UnitType.Terran_Armory) > 0 ) {
									isPossibleToTrain = true;
								}							
							}
							else if(nextUnitTypeToTrain == UnitType.Terran_Battlecruiser) {
								if (myPlayer.completedUnitCount(UnitType.Terran_Starport) > 0 && myPlayer.completedUnitCount(UnitType.Terran_Control_Tower) > 0 
			...
			if (BuildManager.Instance().buildQueue.getItemCount(mySpecialUnitType2) == 0) {	
				...
				else if (mySpecialUnitType2 == UnitType.Terran_Ghost) {
					if (myPlayer.completedUnitCount(UnitType.Terran_Covert_Ops) > 0 && myPlayer.completedUnitCount(UnitType.Terran_Academy) > 0 ) {
						isPossibleToTrain = true;
					}							
				}
	...
	public UnitType getNextCombatUnitTypeToTrain() {
		...
		if (buildOrderArrayOfMyCombatUnitType[nextTargetIndexOfBuildOrderArray] == 1) {
			...
			if(myCombatUnitType3List.size() > 4){ 
				nextUnitTypeToTrain = myCombatUnitType2;
			}
			else {
